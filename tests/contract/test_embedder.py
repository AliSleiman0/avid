...
def embedder(make_embedder: Callable[[], Embedder]) -> Embedder:
    ...
    per test. Both adapters are stateless past that cached session — :class:`FakeEmbedder` holds
    only its dimension count — so sharing one instance across the module is safe. The tests that
    specifically need a *second* instance build their own through ``make_embedder`` rather than
    relying on this fixture's scope (#168)."""
