...
| P8 | **Blocking I/O never touches the event loop.** | `asyncio` debug mode in CI at a 50 ms bar; a slow callback fails the run when it is gross (≥100 ms) or recurs (§14.9, #328). |
...
Enforcement (P8): CI runs the whole test suite with `PYTHONASYNCIODEBUG=1` at a **50 ms** slow-callback threshold. This catches the accidental blocking call *at the moment it's introduced*, which is the only time it's cheap to fix.

**The 50 ms bar detects; it does not convict alone (#328).** A warning fails the run when it is **gross** — ≥ 100 ms, which is asyncio's *own* default threshold, so anything the library would have complained about unprompted — or **corroborated**: the same callback frame grazing twice or more in one session. Everything else is a **graze**, printed in its own block above the verdict and never dropped silently. The reasoning is that blocking I/O is a property of code and therefore recurs, while a shared CI runner descheduling a coroutine for three milliseconds does not pick the same victim twice. ⚠️ The cost is real and bounded: a genuine *one-off* blocking call between 50 and 100 ms now passes. What it bought was a gate whose reds mean something — seven consecutive false positives across seven unrelated PRs (0.052–0.083 s, five different untouched subscribers, zero defects) had made rerunning-without-reading the rational response, which is how a gate stops working. The defects P8 exists for are not marginal: #168's ONNX pool starved the loop for hundreds of milliseconds on *every* embed and the robot went deaf — gross and corroborated many times over. One narrow carve-out (AVID-57): on the Pi, a real device's one-time `start()`/`stop()` (libcamera pipeline init, ALSA card open) is offloaded via `asyncio.to_thread`, so the loop runs no blocking code — but the fixture's own trivial resume callback is measured with inflated wall-clock time while that multi-threaded init contends for the Pi's cores. The gate exempts *only* that fixture setup/teardown boundary, *only* on a real-hardware run; the identical `start`/`stop`/`capture` also run in gated test bodies, so a genuinely loop-blocking adapter still fails, and fake/CI runs stay fully strict (`tests/conftest.py`).
...
⚠️ **That paragraph is true only of a *re*open, and #452 is what the difference cost.** A failed open from a robot that was **not** already degraded absorbs nothing: the machine is in LISTENING or THINKING, not DEGRADED, so no row here applies and the user's later edges land wherever they land. The soak rig sat in THINKING for 24 hours on exactly that arc. The repair is in §6.9 — the first-token deadline is armed on *entry to THINKING* now, so the escape exists whatever opened or failed to open — and it deliberately adds **no row and no trigger here**: `_degrade` remains the sole door into DEGRADED, which is what keeps the "no `audio.playback_*` row in DEGRADED" argument below true.
...
⚠️ **A rejected state transition is a metric, not only a log line (AVID-456).** `StateManager` logs and ignores anything the §3.10.3 table has no rule for, and publishes nothing — so for three milestones the *only* reader was a human tailing journald. The M11 rig spent 32 hours announcing a wedge **153 times in 203 log lines** while `/health`, `/metrics` and every graded soak criterion reported a healthy robot. The registry now carries `illegal_transitions`, counted **per `(state, trigger)` pair** (§9.5 has the shape and the reason it is not a total). The policy is unchanged — counting is not reacting — and what changed is that the question can be asked from outside the process, thirty days later, by something that was not watching at the time.
...
```
0. …and the frame is LOUDER than the echo floor by [gate] barge_in_margin_db -- the RUNNING
   floor while playback is live, the floor FROZEN at the last episode's close for
   [gate] guard_window_ms afterwards (#467)
0b. …and the pre-roll replays only frames not recorded while the robot was audible (#467)
0c. …and the reactive backstop has not stopped the guard from expiring (#467)
...
The floor is adaptive rather than a configured `playback_level × coupling` constant, because while the assistant speaks what the mic hears **is** the echo: tracking it *is* the acoustic-coupling calibration, so speaker volume, mic gain, room and rig geometry all cancel out of the comparison and nothing needs re-measuring when the desk moves. Two rules make it honest — it never adapts on a frame it has judged to be the user (that would raise the bar under the speaker mid-sentence), and it is only ever consulted while the robot is, or has just been, **audible** — so **ordinary turn-taking is never tested against the margin at all.**

⚠️ **AMENDED at #467, and the previous wording is what the defect hid behind.** This section used to say the margin is consulted *"only while the uplink is shut"*, and the code agreed with it literally: `_admits_barge_in` returned **admit** the instant `echo_tail_ms` expired. But the uplink's window and the origin's window are different questions, and only the first of them is about streaming:

| window | question it answers | how long it must be |
|---|---|---|
| `[gate] echo_tail_ms` | may this frame be **sent to the model**? | the DAC's remaining drain |
| `[gate] guard_window_ms` | may this frame **start a turn**? | the room's echo decay |

On the rig the robot re-triggered itself **370 ms** after `audio.playback_finished` — 220 ms past a 150 ms tail — so no margin was consulted, and it did so 26 times in eight minutes for $0.50. Fusing the two windows is what forced the tail to be short: lengthening it to cover a room's decay would also swallow 700 ms of a fast user's reply, cutting the leading phonemes off every prompt answer. Split, each can be right.

**The guard judges against the floor FROZEN at the playback episode's close, never the live floor.** With no playback `EchoFloor` decays toward ambience within a few hundred milliseconds by design, so a *decaying echo* compared against a *decaying floor* clears `floor + margin` trivially — the guard would admit precisely what it exists to refuse. Frozen, the bar stays where the robot's own voice put it: a person at conversational distance clears it, and a fading echo cannot, by construction.

**Both finalizers arm the guard; only `end_response` arms the streaming tail.** `interrupt()` deliberately sets no tail (`Speaker.stop` closes the handle, so ALSA drops the buffer, and holding the uplink shut would clip the very words that interrupted) — but it must still arm the *guard*, because a speaker that was audible a moment ago is audible whether it stopped early or late. A self-triggered barge-in is exactly the case where the guard matters and it was the one case that skipped it. Both are therefore armed inside `_take_playback`, the single-taker both finalizers pass through, so the property is structural rather than a rule to remember at two call sites.

⚠️ **`echo_tail_ms` is checked against the speaker, not trusted.** `Speaker.play()` reports frames the device **accepted into its ring buffer**, so `end_response` fires up to one buffer depth before the DAC has emitted the reply (§6.2.4 step 3's residual, and SDS:1084). The shipped 150 ms therefore carried ~43 ms of real slack at 24 kHz. `Config` now computes the depth from `[speaker] sample_rate` and refuses to load a tail that cannot cover it plus 100 ms for the room — a *computed* relation rather than a comment, because a restated "107 ms" is drift with a delay fuse.

**Step 0b — the pre-roll replays only frames recorded while the robot was inaudible.** §6.3's ring buffer is fed on **every** captured frame, the robot's own voice included, and the replay used to be unconditional. So a single frame that scraped past the margin did not send one frame of echo to the model — it sent up to a full pre-roll of the robot's *contiguous* speech, labelled as the user's utterance, which is comfortably enough for the server to transcribe and answer. That is the amplifier that turned one false admit into a conversation. The ring now records each frame's verdict and drains only the **trailing contiguous run** of non-echo frames, which is exactly the user's words in a genuine mid-reply barge-in (AVID-161 keeps its leading phonemes: those frames cleared the margin, so they are not flagged, and they are the ones adjacent to the rising edge).

**Step 0c — the reactive backstop.** Nothing in this system rate-limits a *reactive* turn: `evaluate_policy` has one call site and it is the proactive path (§10.4), so quiet hours, the cooldown and the daily budget never see a turn a human — or an echo — started. ⚠️ **A turn-rate cap cannot close that hole.** The measured runaway ran at **3.25 turns/min** and a fast human exchange with this robot is **5–6/min**: it was *slower than a conversation*, so no rate threshold separates the two populations at any value, and one tight enough to catch the defect would silence a chatty owner. What separates them is the **gap to the robot's own reply** — 370 ms for the runaway, against a person who has to hear the reply end before answering. So `[gate] reactive_budget` counts only origins arriving within `reactive_back_to_back_s` of an episode close, and once the budget is spent inside `reactive_window_s` the guard **stops expiring**: every origin must then clear the frozen floor however long ago playback ended, until a full window passes without one. It is a cap on turns that never proved they came from a human, not a cap on turns.

**Refusals are counted, per rule.** `avid.domain.audio.ADMISSION_RULES` pins the vocabulary the way §14.2 pins `POLICY_RULES`, and the same strings are the keys of `admission_refusals` on `/metrics` (§3.12.2). Per-rule rather than a total, because a genuinely loud room and a robot re-triggering itself want opposite responses. ⚠️ The gate's own log line also now reports **how many frames were tested**, not only how many were suppressed: for eight minutes on 2026-08-24 it printed `0 suppressed` while the robot answered itself, because *a gate that refuses nothing and a gate that never runs print identically*. That is CLAUDE.md §7.1 — report the quantity you grade — applied to an instrument that had been quietly failing it.
...
| Held sessions answer | Measured: a session held silent for 64 s answered a text turn in **277 ms** to `response.created` and **389 ms** to first audio — the same first-token figures as a fresh session (§6.10.5) |
...
`ConversationService` arms the deadline on **entry to THINKING** — every entry, whichever path drove it — and cancels it on **exit**, by a synchronous observer registered with `StateManager.watch` at the composition root. Duration is `[gate] think_timeout_s`, default **10 s**, and it **must be less than `session_idle_close_s`** — asserted in `Config`, because the idle close tears the socket down without transitioning, so a larger value silently never fires and restores the wedge exactly. The turn's first assistant delta still cancels it directly: that is where "first token" is *defined*, and it closes the window between the delta and `audio.playback_started`.

⚠️ **It was armed at the local VAD's falling edge until #452, and the difference cost 24 hours on the soak rig.** The arming lived on a *code path* while the thing it protects is a *state*, and three reachable arcs enter THINKING without reaching that path — only one of which involves a network fault at all:

- **A refused reactive `open()`.** `AudioService` drives `LISTENING → THINKING` off its own falling edge whether or not anything downstream can act on it. `ConversationService._on_speech_started` catches the `OSError`, logs it and returns without touching the machine — so the falling-edge handler returns at its `_session_open` guard *before* the arm site. Found on the M11 window: the robot entered THINKING at minute 3, never left, and **147 of the process's 175 log lines** were `ignored illegal transition … in state THINKING`. That is 8,600× the designed bound, and it is the same fault §6.9 already records at 54 seconds — the version that got a rule written for it.
- **A refused proactive `open()`, which is worse.** `BehaviorService` drives `IDLE → THINKING` *before* publishing `behavior.trigger_fired`, so there is no falling edge coming at all, ever: nobody is in the room.
- **Plain cross-subscriber ordering, on a healthy network.** The bus is FIFO per subscriber and not across (§9.1.2), so `audio.speech_ended` can be delivered before its own `audio.speech_started` has opened the session.

The semantics widen with the arming site, and the honest statement of the criterion is now **"the machine has been in THINKING for `think_timeout_s`"** rather than "no first token that long after the falling edge" — a strict superset. Two consequences are worth writing down. The timer's old `_session_open` guard is **deleted**: every wedge above has no session by construction, so that guard declined to fire in exactly the case that motivated the timer, and moving the arm site without deleting it would have been inert. And `_teardown_locked` no longer cancels the deadline — only `stop()` does — because a teardown that cancelled it would re-open the hole: an idle close fires with the machine still in THINKING and drives no transition, so nothing would re-arm what it cancelled.

⚠️ **A direct call, not a `state.transitioned` subscription** (§9.1.4). That subscription is at-most-once behind a bounded DROP_OLDEST queue, so a single overflow would silently drop an arming and restore the wedge — and an invariant that survives only while a queue has room is not an invariant. `StateManager.watch` is therefore the second public method on that class beyond the Protocol, for the same reason `transition` is the first.
...
- **THINKING cannot be occupied without an armed exit** (#452). Every other row out of THINKING needs either a live session (`audio.playback_started`, `audio.playback_finished`, `conversation.session_lost`) or the human to speak again (`audio.speech_started`); this deadline is the only escape that needs neither, which is what makes arming it an invariant rather than a nicety. Asserted off-hardware on all three arcs above, and at full stack in `tests/e2e/test_m5_gate.py` — the only place the "no `ignored illegal transition`" assertion works, because illegal transitions are reachable only when the real `AudioService` drives the audio edges. ⚠️ Expressing the failed open *there* required the replay fake to be able to refuse a connect at all (§14.3); until #452 it could not, so the one test that would have caught this could not be written.
...
Public methods beyond this Protocol exist only where §9.1.4 requires a direct call — `MemoryService.store_fact/retrieve/forget`, `StateManager.transition`, `StateManager.watch` (#452: a reaction that must fire on **every** entry into a state cannot ride `state.transitioned`, which is at-most-once behind a bounded DROP_OLDEST queue — one overflow would silently drop an arming), `AffectService.set_affect`, `MotionService.look_at` (M9's `GestureTools` surface, §3.9.1). Everything else is bus-mediated.
...
| F-2 | Model never produces a first token — **or the session never opened at all** (#452) | Turn hangs | High | §6.9 think timeout → DEGRADED via the same `_degrade` helper as F-1, armed on entry to THINKING so every path in has a way out | `THINK_TIMEOUT` trigger |
...
**A refused connect is a constructor knob, not a manifest record (#452).** `ReplayRealtimeClient(open_error=…)` makes `open()` raise `OSError` — the port's documented transport-failure contract (§3.9.1) — and it is public and mutable so a test can clear it mid-run to script *refuses, then recovers*. Deliberately **not** a new record type with a `format` bump: a manifest describes the stream on an **open** session, so `--capture` could never record a connect that never happened, and a bump would invalidate every committed `assets/sessions/` directory for a fault that belongs to the transport rather than to the recording. It exists because it had to: a failed open was expressible only from a test-local subclass in `tests/services/`, which is precisely where the "no `ignored illegal transition`" assertion does not work — so the e2e gate was structurally incapable of reaching a defect that then ran for 24 hours on the rig. **A fake that cannot fail the way the real transport routinely does is an incomplete port** (P6).
